    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const musicResponse = await axios.get("/music"); // Gọi API để lấy tất cả bài hát
                ...
                if (musicResponse?.result?.data) {
                    setMusics(musicResponse.result.data); // Cập nhật danh sách bài hát
                }
            } catch (error) {
            ...
            <div className={styles.container}>
                    ...
                    {musics && musics.length > 0 ? (
                        musics.map(music => (
                            <option
                                ...
                                className={album.musics.map(song => song.id_music).includes(music.id_music) ? "selected" : ""}
                            >
                                {music.name}
                            </option>
                        ))
                ...
                {selectedSongs.length > 0 ? (
                    selectedSongs.map(songId => {
                        const selectedSong = musics.find(music => music.id_music === songId);
                        return selectedSong ? (
                            <div key={songId} className={styles.songGroup}>
                                <span>{selectedSong.name}</span>
                                <button onClick={() => handleSongSelect(songId)}>Bỏ chọn</button>
                            </div>
                        ) : null;
                    })
                ) : (
